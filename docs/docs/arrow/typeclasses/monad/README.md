...
`Monad` includes all combinators present in [`Applicative`]({{ '/arrow/typeclasses/applicative/' | relative_url }}).
...
The [`Eval`]({{ '/arrow/core/eval' | relative_url }}) variant allows you to pass lazily calculated values.
