...
If you would like to know about using the [`Reader`]({{ '/arrow/data/reader' | relative_url }}) datatype instead, visit [this article](https://medium.com/@JorgeCastilloPr/kotlin-dependency-injection-with-the-reader-monad-7d52f94a482e) by [Jorge Castillo](https://github.com/JorgeCastilloPrz).
...
The technique demonstrated below to write polymorphic code is available for all other typeclasses besides [`Functor`]({{ '/arrow/typeclasses/functor' | relative_url }}).
